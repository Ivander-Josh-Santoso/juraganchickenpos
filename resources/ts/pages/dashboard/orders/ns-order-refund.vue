<template>
    <div class="-m-4 flex-auto flex flex-wrap relative">
        ...
        <div class="px-4 w-full lg:w-1/2">
            <h3 class="py-2 border-b-2 text-gray-700 border-blue-400">{{ __( 'Refund Dengan Produk' ) }}</h3>
            <div class="my-2">
                <ul>
                        ...
                        <div class="flex-auto flex-col flex">
                            ...
                            <div class="flex justify-between p-2">
                                <div class="flex items-center text-gray-700">
                                    <span v-if="order.shipping > 0" class="mr-2">{{ __( 'Refund Ongkir' ) }}</span>
                                    <ns-checkbox v-if="order.shipping > 0" @change="toggleRefundShipping( $event )" :checked="refundShipping"></ns-checkbox>
                                </div>
                                <div>
                                    <button @click="addProduct()" class="border-2 rounded-full border-gray-200 px-2 py-1 hover:bg-blue-400 hover:text-white text-gray-700">{{ __( 'Tambah Produk' ) }}</button>
                                </div>
                            </div>
                        </div>
                    </li>
                    <li>
                        <h4 class="py-1 border-b-2 text-gray-700 border-blue-400">{{ __( 'Daftar Produk' ) }}</h4>
                    </li>
                        ...
                        <div class="px-2 text-gray-700 flex justify-between flex-auto">
                            <div class="flex flex-col">
                                <p class="py-2">
                                    ...
                                    <span v-if="product.condition === 'damaged'" class="rounded-full px-2 py-1 text-xs bg-red-400 mx-2 text-white">{{ __( 'Rusak' ) }}</span>
                                    <span v-if="product.condition === 'unspoiled'" class="rounded-full px-2 py-1 text-xs bg-green-400 mx-2 text-white">{{ __( 'Baik' ) }}</span>
                                </p>
        ...
        <div class="px-4 w-full lg:w-1/2">
            <h3 class="py-2 border-b-2 text-gray-700 border-blue-400">{{ __( 'Ringkasan' ) }}</h3>
            <div class="py-2">
                ...
                <div @click="selectPaymentGateway()" class="bg-indigo-400 text-white font-semibold flex mb-2 p-2 justify-between cursor-pointer">
                    ...
                    <span>{{ selectedPaymentGateway ? selectedPaymentGateway.label : 'Tidak ada' }}</span>
                </div>
            ...
            selectFields: [
                {
                    ...
                    name: 'product_id',
                    label: __( 'Produk' ),
                    description: __( 'Pilih produk yang akan direfund.' )
                }
            ]
        }
    }, 
    methods: {
        ...
        proceedPayment() {
            if ( this.selectedPaymentGateway === false ) {
                return nsSnackBar.error( __( 'Silakan pilih metode pembayaran terlebih dahulu.' ) ).subscribe();
            }
            if ( this.total === 0 ) {
                return nsSnackBar.error( __( 'Tidak ada produk yang bisa direfund.' ) ).subscribe();
            }
            if ( this.screenValue === 0 ) {
                return nsSnackBar.error( __( 'Silakan masukkan nominal refund yang valid.' ) ).subscribe();
            }
            Popup.show( nsPosConfirmPopupVue, {
                title: __( 'Konfirmasi Aksi Anda' ),
                message: __( 'Refund akan diproses pada pesanan ini.' ),
                onAction: ( action ) => {
                    if ( action ) {
                        this.doProceed();
                    }
        ...
        addProduct() {
            ...
            if ( ! this.formValidation.fieldsValid( this.selectFields ) ) {
                return nsSnackBar.error( __( 'Silakan pilih produk terlebih dahulu.' ) ).subscribe();
            }
            ...
            if ( existingProducts.length > 0 ) {
                ...
                if ( totalUsedQuantity === currentProduct[0].quantity ) {
                    return nsSnackBar.error( __( 'Jumlah produk tidak mencukupi.' ) ).subscribe();
                }
            }
        ...
        deleteProduct( product ) {
            const promise = new Promise( ( resolve, reject ) => {
                Popup.show( nsPosConfirmPopupVue, {
                    title: __( 'Konfirmasi Aksi Anda' ),
                    message: __( 'Apakah Anda yakin ingin menghapus produk ini?' ),
                    onAction: action => {
                        if ( action ) {
