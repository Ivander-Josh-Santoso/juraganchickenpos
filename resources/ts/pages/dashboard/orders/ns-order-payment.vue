    ...
    <div class="flex -mx-4 flex-wrap">
      <div class="px-2 w-full mb-4 md:w-1/2">
        ...
        <div v-if="order.payment_status === 'paid'" class="flex items-center justify-center h-full">
          <h3 class="text-gray-700 font-semibold">{{ __('Pesanan yang sudah lunas tidak bisa dibayar lagi.') }}</h3>
        </div>
      </div>
      <div class="px-2 w-full mb-4 md:w-1/2">
...
export default {
  ...
  methods: {
    ...
    loadPaymentFields() {
      nsHttpClient.get('/api/nexopos/v4/orders/payments').subscribe((fields) => {
    ...
    submitPayment(value) {
      ...
      Popup.show(nsPosConfirmPopupVue, {
        ...
        onAction: (action) => {
          if (action) {
            nsHttpClient.post(`/api/nexopos/v4/orders/${this.order.id}/payments`, form).subscribe(
              (result) => {
