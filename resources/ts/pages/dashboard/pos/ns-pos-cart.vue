      ...
      <div class="cart-table flex flex-auto flex-col overflow-hidden">
        ...
        <div id="cart-products-table" class="flex flex-auto flex-col overflow-auto">
          ...
          <div class="text-gray-700 flex" v-if="products.length === 0">
            <div class="w-full text-center py-4 border-b border-gray-200">
              <h3 class="text-gray-600">{{ __('Belum ada produk yang ditambahkan...') }}</h3>
            </div>
          </div>
            ...
            <div class="w-full lg:w-4/6 p-2 border border-l-0 border-t-0 border-gray-200">
              ...
              <div class="flex justify-between product-controls">
                <div class="-mx-1 flex flex-wrap">
                  ...
                  <div class="px-1 w-1/2 md:w-auto mb-1">
                    <a @click="openDiscountPopup(product, 'product')" class="hover:text-blue-400 cursor-pointer outline-none border-dashed py-1 border-b border-blue-400 text-sm"
                      >{{ __('Diskon') }} <span v-if="product.discount_type === 'percentage'">{{ product.discount_percentage }}%</span> : {{ product.discount | currency }}</a
                    >
                  </div>
        ...
        <div id="cart-products-summary" class="flex">
          <table class="table w-full text-sm text-gray-700" v-if="visibleSection === 'both'">
            <tr>
              <td width="200" class="border border-gray-300 p-2">
                <a @click="selectCustomer()" class="hover:text-blue-400 cursor-pointer outline-none border-dashed py-1 border-b border-blue-400 text-sm">Nama Pelanggan : {{ customerName }}</a>
              </td>
          ...
          <table class="table w-full text-sm text-gray-700" v-if="visibleSection === 'cart'">
            <tr>
              <td width="200" class="border border-gray-300 p-2">
                <a @click="selectCustomer()" class="hover:text-blue-400 cursor-pointer outline-none border-dashed py-1 border-b border-blue-400 text-sm">{{ __('Nama Pelanggan :') }} {{ customerName }}</a>
              </td>
              <td width="200" class="border border-gray-300 p-2">
        ...
        <div class="h-16 flex flex-shrink-0 border-t border-gray-200" id="cart-bottom-buttons">
          ...
          <div
            ...
            id="discount-button"
            class="flex-shrink-0 w-1/4 flex items-center font-bold cursor-pointer justify-center bg-white border-r border-gray-200 hover:bg-indigo-100 flex-auto text-gray-700"
          >
            <i class="mr-2 text-2xl lg:text-xl las la-percent"></i>
            <span class="text-lg hidden md:inline lg:text-2xl">{{ __('Diskon') }}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    async holdOrder() {
      ...
      const queues = nsHooks.applyFilters('ns-hold-queue', [ProductsQueue, CustomerQueue, TypeQueue]);
    ...
    async payOrder() {
      const queues = nsHooks.applyFilters('ns-pay-queue', [ProductsQueue, CustomerQueue, TypeQueue, PaymentQueue]);
