  ...
  methods: {
    ...
    proceedAddingPayment(event) {
      ...
      if (payments.filter((p) => p.identifier === 'account-payment').length > 0) {
        return nsSnackBar.error(__('The customer account can only be used once per order. Consider deleting the previously used payment.')).subscribe();
      }

      if (value > this.order.customer.account_amount) {
        return nsSnackBar
          .error(
            __('Not enough funds to add {amount} as a payment. Available balance {balance}.')
