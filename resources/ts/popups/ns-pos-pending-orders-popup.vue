    ...
    <div class="shadow-lg bg-white w-6/7-screen md:w-3/5-screen lg:w-2/5-screen h-6/7-screen flex flex-col overflow-hidden">
        ...
        <div class="p-2 flex overflow-hidden flex-auto">
            <ns-tabs :active="active" @changeTab="setActiveTab( $event )">
                <ns-tabs-item identifier="ns.hold-orders" :label="__( 'Ditahan' )" padding="p-0" class="flex flex-col overflow-hidden">
                ...
                <ns-tabs-item identifier="ns.unpaid-orders" :label="__( 'Belum Dibayar' )" padding="p-0" class="flex flex-col overflow-hidden">
                    <ns-pos-pending-orders :orders="orders" 
                        @searchOrder="searchOrder( $event )"
                        @previewOrder="previewOrder( $event )"
                        @printOrder="printOrder( $event )"
                ...
                <ns-tabs-item identifier="ns.partially-paid-orders" :label="__( 'Dibayar Sebagian' )" padding="p-0" class="flex flex-col overflow-hidden">
                    <ns-pos-pending-orders :orders="orders" 
                        @searchOrder="searchOrder( $event )"
                        @previewOrder="previewOrder( $event )"
                        @printOrder="printOrder( $event )"
