  ...
  <div class="shadow-lg bg-white w-95vw md:w-3/6-screen lg:w-2/6-screen">
    ...
    <div class="p-1">
      <ns-tabs @changeTab="setActiveTab($event)" :active="activeTab">
        <ns-tabs-item :label="__('Apply A Coupon')" padding="p-2" identifier="apply-coupon">
            ...
            <div class="pt-2 fade-in-entrance anim-duration-500 overflow-y-auto h-64" v-if="customerCoupon">
              <table class="w-full">
                <thead>
                  ...
                  <tr>
                    <td class="p-2 w-1/2 text-gray-700 border border-gray-200">{{ __('Diskon') }} ({{ getCouponType(customerCoupon.coupon.type) }})</td>
        ...
        <ns-tabs-item :label="__('Active Coupons')" padding="p-1" identifier="active-coupons">
          <ul v-if="order">
            ...
            <li v-if="order.coupons.length === 0" class="flex justify-between bg-gray-100 items-center p-2">No coupons applies to the cart.</li>
          </ul>
        </ns-tabs-item>
      </ns-tabs>
    </div>
  ...
  mounted() {
    ...
    if (this.$popupParams) {
      ...
      if (this.$popupParams.apply_coupon) {
        ...
        this.getCoupon(this.couponCode).subscribe({
          next: (coupon) => {
  ...
  methods: {
    ...
    apply() {
      try {
        ...
        if (this.customerCoupon.coupon.valid_hours_start !== null && !ns.date.moment.isAfter(this.customerCoupon.coupon.valid_hours_start) && this.customerCoupon.coupon.valid_hours_start.length > 0) {
          return nsSnackBar.error(__('The coupon is out from validity date range.')).subscribe();
        }

        if (this.customerCoupon.coupon.valid_hours_end !== null && !ns.date.moment.isBefore(this.customerCoupon.coupon.valid_hours_end) && this.customerCoupon.coupon.valid_hours_end.length > 0) {
          return nsSnackBar.error(__('The coupon is out from validity date range.')).subscribe();
        }
        ...
        if (requiredProducts.length > 0) {
          ...
          if (this.order.products.filter((p) => productIds.includes(p.product_id)).length === 0) {
            return nsSnackBar.error(__("This coupon requires products that aren't available on the cart at the moment.")).subscribe();
          }
        }
        ...
        if (requiredCategories.length > 0) {
          ...
          if (this.order.products.filter((p) => categoriesIds.includes(p.$original().category_id)).length === 0) {
            return nsSnackBar.error(__("This coupon requires products that belongs to specific categories that aren't included at the moment.").replace('%s')).subscribe();
          }
        }
        ...
        nsSnackBar.success(__('The coupon has applied to the cart.')).subscribe();
      } catch (exception) {
    ...
    getDiscountValue(coupon) {
      switch (coupon.type) {
        ...
        case 'flat_discount':
          return this.$options.filters.currency(coupon.discount_value);
      }
    },
