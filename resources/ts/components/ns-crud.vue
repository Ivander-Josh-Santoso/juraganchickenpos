...
<template>
    <div id="crud-table" class="w-full rounded-lg bg-white" :class="mode !== 'light' ? 'shadow mb-8': ''">
            ...
            <div id="crud-search-box" class="w-full md:w-auto -mx-2 mb-2 md:mb-0 flex">
                ...
                <div class="px-2 flex" v-if="showQueryFilters">
                    <button @click="openQueryFilter()" :class="withFilters ? 'bg-blue-100 border-blue-200' : 'text-gray-800 border-gray-400'" class="border rounded-full hover:border-blue-400 hover:text-white hover:bg-blue-400 text-sm h-10 px-3 outline-none bg-white">
                        <i v-if="! withFilters" class="las la-filter"></i>
                        <i v-if="withFilters" class="las la-check"></i>
                        <span v-if="! withFilters">{{ __( 'Filter' ) }}</span>
                        <span v-if="withFilters">{{ __( 'Ada Filter' ) }}</span>
                    </button>
                </div>
            </div>
            <div id="crud-buttons" class="-mx-1 flex flex-wrap w-full md:w-auto">
                <div class="px-1 flex" v-if="selectedEntries.length > 0">
                    <button @click="clearSelectedEntries()" class="flex justify-center items-center rounded-full text-sm h-10 px-3 hover:border-blue-400 hover:text-white hover:bg-blue-400 outline-none border-gray-400 border text-gray-700">
                        <i class="lar la-check-square"></i> {{ __( '{entries} entri dipilih' ).replace( '{entries}', selectedEntries.length ) }}
                    </button>
                </div>
                <div class="px-1 flex">
                    <button @click="downloadContent()" class="flex justify-center items-center rounded-full text-sm h-10 px-3 bg-teal-400 outline-none text-white font-semibold"><i class="las la-download"></i> {{ __( 'Unduh' ) }}</button>
                </div>
            </div>
        </div>
        <div class="flex">
            <div class="overflow-x-auto flex-auto">
                <table class="table w-full" v-if="Object.values( columns ).length > 0">
                    ...
                    <tbody>
                        ...
                        <tr v-if="! result || result.data.length === 0">
                            <td :colspan="Object.values( columns ).length + 2" class="text-center text-gray-600 py-3">{{ __( 'Tidak ada data untuk ditampilkan...' ) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="p-2 flex flex-col md:flex-row justify-between">
            <div v-if="bulkActions.length > 0" id="grouped-actions" class="mb-2 md:mb-0 flex justify-between rounded-full bg-gray-200 p-1">
                <select class="text-gray-800 outline-none bg-transparent" v-model="bulkAction" id="grouped-actions">
                    <option selected value=""><slot name="bulk-label">{{ __( 'Aksi Massal' ) }}</slot></option>
                    <option :key="index" v-for="(action, index) of bulkActions" :value="action.identifier">{{ action.label }}</option>
                </select>
                <button @click="bulkDo()" class="h-8 w-8 outline-none hover:bg-blue-400 hover:text-white rounded-full bg-white flex items-center justify-center"><slot name="bulk-go">{{ __( 'Jalankan' ) }}</slot></button>
            </div>
            <div class="flex">
                <div class="items-center flex text-gray-600 mx-4">{{ resultInfo }}</div>
                <div id="pagination" class="flex -mx-1">
...
export default {
    ...
    computed: {
        ...
        resultInfo() {
            return __( 'menampilkan {perPage} dari {items} data' )
                .replace( '{perPage}', this.result.total )
                .replace( '{items}', this.result.total )
        }
    },
    methods: {
        ...
        downloadContent() {
            nsHttpClient.post( `${this.src}/export?${this.getParsedSrc}`, { entries : this.selectedEntries.map( e => e.$id ) })
                .subscribe( (result: any) => {
                    setTimeout( () => document.location   =   result.url, 300 );
                    nsSnackBar
                        .success( __( 'Dokumen berhasil dibuat.' ) )
                        .subscribe()
                }, error => {
                    nsSnackBar
                        .error( error.message || __( 'Terjadi kesalahan tak terduga.' ) )
                        .subscribe();
                })
        },
        ...
        clearSelectedEntries() {
            Popup.show( nsPosConfirmPopupVue, {
                title: __( 'Hapus entri terpilih?' ),
                message: __( 'Apakah Anda ingin menghapus semua entri yang dipilih?' ),
                onAction: ( action ) => {
                    if ( action ) {
                        this.selectedEntries    =   [];
                    }
        ...
        bulkDo() {
            if ( this.bulkAction ) {
                if ( this.selectedEntries.length > 0 ) {
                    if ( confirm( this.getSelectedAction.confirm || this.$slots[ 'error-bulk-confirmation' ] || __( 'Apakah Anda ingin menjalankan aksi massal pada entri yang dipilih?' ) ) ) {
                        return nsHttpClient.post( `${this.src}/bulk-actions`, {
                            action: this.bulkAction,
                            entries: this.selectedEntries.map( r => r.$id )
                        }).subscribe({
                    ...
                    return nsSnackBar.error( this.$slots[ 'error-no-selection' ] ? this.$slots[ 'error-no-selection' ][0].text : __( 'Belum ada entri yang dipilih.' ) )
                        .subscribe();
                }
            } else {
                return nsSnackBar.error( this.$slots[ 'error-no-action' ] ? this.$slots[ 'error-no-action' ][0].text : __( 'Belum ada aksi yang dipilih.' ) )
                    .subscribe();
            }
