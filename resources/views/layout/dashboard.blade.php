...
<body <?php echo in_array( app()->getLocale(), config( 'nexopos.rtl-languages' ) ) ? 'dir="rtl"' : "";?>>
        ...
        <div id="dashboard-body" class="overflow-hidden flex flex-auto">
            <div id="dashboard-aside" v-cloak v-if="sidebar === 'visible'" class="w-64 z-50 absolute md:static flex-shrink-0 bg-gray-900 h-full flex-col overflow-hidden">
                <div class="overflow-y-auto h-full text-sm">
                    <div class="logo py-4 flex justify-center items-center">
                        ...
                        @else
                        <h1 class="font-black text-transparent bg-clip-text bg-gradient-to-b from-blue-200 to-indigo-400 text-3xl">JuraganChicken</h1>
                        @endif
                    </div>
                    <ul>
            ...
            <div class="flex flex-auto flex-col overflow-hidden bg-gray-200">
                ...
                <div class="p-2 text-xs flex justify-end text-gray-500">
                    {!! 
                        Hook::filter( 'ns-footer-signature', sprintf( __( 'You\'re using <a tager="_blank" href="%s" class="hover:text-blue-400 mx-1 inline-block">Juragan Chicken Pos %s</a>' ), 'https://www.instagram.com/immanuel_ivander_/', config( 'nexopos.version' ) ) )
                    !!}
                </div>
            </div>
        </div>
    ...
    @section( 'layout.dashboard.footer' )
        ...
        <script src="{{ asset('js/billing-shipping.js') }}"></script>
    @show
</body>
</html>