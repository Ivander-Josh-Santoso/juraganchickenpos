                ...
                <div class="flex -mx-2" v-if="timezone !== ''">
                    ...
                    <div class="px-2 flex">
                        <button @click="recomputeForSpecificYear()" class="rounded flex justify-between bg-white shadow py-1 items-center text-gray-700 px-2">
                            <i class="las la-sync-alt text-xl"></i>
                            <span class="pl-2">{{ __( 'Hitung Ulang' ) }}</span>
                        </button>
                    </div>
                </div>
                    ...
                    <div class="flex w-full">
                        <div class="my-4 flex justify-between w-full">
                            <div class="text-gray-600">
                                <ul>
                                    <li class="pb-1 border-b border-dashed border-gray-200">{{ sprintf( __( 'TANGGAL : %s' ), ns()->date->getNowFormatted() ) }}</li>
                                    <li class="pb-1 border-b border-dashed border-gray-200">{{ __( 'DOKUMEN : LAPORAN TAHUNAN' ) }}</li>
                                    <li class="pb-1 border-b border-dashed border-gray-200">{{ sprintf( __( 'Dibuat Oleh : %s' ), Auth::user()->username ) }}</li>
                                </ul>
                            </div>
                            <div>
                            ...
                            <table class="table w-full">
                                <thead class="text-gray-700">
                                    <tr>
                                        ...
                                        <th width="150" class="bg-gray-100 text-gray-700 border border-gray-300 p-2 text-right">{{ __( 'Penjualan' ) }}</th>
                                        <th width="150" class="bg-gray-100 text-gray-700 border border-gray-300 p-2 text-right">{{ __( 'Pajak' ) }}</th>
                                        <th width="150" class="bg-gray-100 text-gray-700 border border-gray-300 p-2 text-right">{{ __( 'Pengeluaran' ) }}</th>
                                        <th width="150" class="bg-gray-100 text-gray-700 border border-gray-300 p-2 text-right">{{ __( 'Pendapatan' ) }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
