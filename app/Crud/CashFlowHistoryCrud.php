...
class CashFlowHistoryCrud extends CrudService
{
    ...
    {
        return [
            'list_title'             =>  __( 'Daftar Arus Kas' ),
            'list_description'       =>  __( 'Menampilkan semua Arus Kas.' ),
            'no_entry'               =>  __( 'Belum ada Arus Kas yang terdaftar' ),
            'create_new'             =>  __( 'Tambah Arus Kas baru' ),
            'create_title'           =>  __( 'Buat Arus Kas baru' ),
            'create_description'     =>  __( 'Daftarkan Arus Kas baru dan simpan.' ),
            'edit_title'             =>  __( 'Edit Arus Kas' ),
            'edit_description'       =>  __( 'Ubah Arus Kas.' ),
            'back_to_list'           =>  __( 'Kembali ke Riwayat Pengeluaran' ),
        ];
    }
                ...
                'general'   =>  [
                    ...
                    'fields'    =>  [
                        [
                            'type'  =>  'text',
                            'name'  =>  'expense_id',
                            'label' =>  __( 'ID Pengeluaran' ),
                            'value' =>  $entry->expense_id ?? '',
                        ], [
                            'type'  =>  'text',
                            'name'  =>  'expense_name',
                            'label' =>  __( 'Nama Pengeluaran' ),
    ...
    public function beforeDelete( $namespace, $id, $model ) {
        if ( $namespace == 'ns.cash-flow-history' ) {
             ...
             *  return response([
             *      'status'    =>  'danger',
             *      'message'   =>  __( 'Anda tidak diizinkan melakukan hal ini.' )
             *  ], 403 );
             **/
            ...
            if ( $model->status !== CashFlow::STATUS_ACTIVE ) {
                throw new NotAllowedException( __( 'Riwayat pengeluaran ini tidak memiliki status yang mengizinkan penghapusan.' ) );
            }
            ...
            return [
                'status'    =>  'success',
                'message'   =>  __( 'Riwayat pengeluaran akan dihapus.' )
            ];
        }
    }
    ...
    public function setActions( $entry, $namespace )
    {
        ...
        switch( $entry->operation ) {
            case CashFlow::OPERATION_CREDIT : 
                $entry->operation             =   "<span class='bg-green-400 text-white rounded-full px-2 py-1 text-sm'>" . __( 'Kredit' ) . '</span>';
            break;
            case CashFlow::OPERATION_DEBIT : 
