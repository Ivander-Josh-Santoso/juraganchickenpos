...
class ProcurementController extends DashboardController
{
    ...
    public function edit( Procurement $procurement, ProcurementRequest $request )
    {
        if ( $procurement->delivery_status === Procurement::STOCKED ) {
            throw new NotAllowedException( __( 'Tidak dapat mengubah pengadaan yang sudah masuk stok. Silakan lakukan penyesuaian stok atau hapus pengadaan tersebut.' ) );
        }
    ...
    public function editProduct( Request $request, $procurement_id, $product_id )
    {
        ...
        throw new AccessDeniedException( 
            sprintf( 
                __( 'Produk dengan ID %s tidak termasuk dalam pengadaan dengan ID %s' ),
                $product_id,
                $procurement_id
            )
        );
    ...
    /**
     * Refresh a specific procurement manually
     * @param int procurement id
     * @return array response
     */
    ...
    public function createProcurement()
    {
        ...
        return $this->view( 'pages.dashboard.procurements.create', [
            'title'         =>  __( 'Pengadaan Baru' ),
            'description'   =>  __( 'Buat pengadaan baru' )
        ]);
    }
    ...
    public function updateProcurement( Procurement $procurement )
    {
        ...
        if ( $procurement->delivery_status === Procurement::STOCKED ) {
            throw new NotAllowedException( __( 'Tidak dapat mengubah pengadaan yang sudah masuk stok. Silakan lakukan penyesuaian atau hapus pengadaan.' ) );
        }

        return $this->view( 'pages.dashboard.procurements.edit', [
            'title'         =>  __( 'Ubah Pengadaan' ),
            'description'   =>  __( 'Lakukan penyesuaian pada pengadaan yang sudah ada.' ),
    ...
    public function procurementInvoice( Procurement $procurement )
    {
        ...
        return $this->view( 'pages.dashboard.procurements.invoice', [
            'title'         =>  sprintf( __( '%s - Faktur' ), $procurement->name ),
            'description'   =>  __( 'Daftar produk yang telah dipesan.' ),
            'procurement'   =>  $procurement,
